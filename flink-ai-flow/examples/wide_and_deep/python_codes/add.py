...
def map_func(context):
    ...
    logging.info("cluster:" + str(cluster_json))
    logging.info("job name:" + job_name)
    logging.info("current index:" + str(index))
    logging.info("gpu info: " + gpu_info)
    ...
    if 'ps' == job_name:
        ...
        with tf.device(tf.train.replica_device_setter(worker_device='/job:worker/task:' + str(index), cluster=cluster)):
            ...
            logging.info("python worker index:" + str(index))
            sys.stdout.flush()
            try:
                hooks = [tf.train.StopAtStepHook(last_step=60)]
                with tf.train.MonitoredTrainingSession(is_chief=is_chief, master=server.target, config=sess_config,
                                                       checkpoint_dir="/tmp/ckpt" + str(t),
                                                       hooks=hooks) as mon_sess:
                    while not mon_sess.should_stop():
                        logging.info(mon_sess.run(train_ops, feed_dict={a: [1.0, 2.0, 3.0]}))
                        sys.stdout.flush()
                        time.sleep(1)
            finally:
